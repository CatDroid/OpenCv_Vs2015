...
//            this->eigenvectors.release();
		...
		// regression 岭回归 正则化最小二乘  L1、L2正则化约束 假设参数W 的先验分布为 Wi ~ N(0, σ^2 )
		// http://www.jianshu.com/p/f71848c7aaf3 
		// http://blog.csdn.net/google19890102/article/details/27228279 
		// w =  (X.t * X + λ*I ).inv * X.t * y

		//  LASSO 回归（L1 正则化）  参数W 的先验分布为 Wi ~ Laplace(0, σ^2)  ...
		...
		/*
			...
			OpenCV 3.0中实现矩阵求逆有四种方法（LU、cholesky、eig以及SVD）
...
int ldmarkmodel::track(const cv::Mat& src, cv::Mat& current_shape, bool isDetFace){
	...
	printf(" track LinearRegressors.size() %d \n" , LinearRegressors.size() );
    if(src.channels() == 1){
        grayImage = src;
    }else if(src.channels() == 3){
        cv::cvtColor(src, grayImage, CV_BGR2GRAY);
...
//    }
    ...
    if(isDetFace || faceBox.area()<100){
        ...
        if(mFaceRects.size() <=0){
            current_shape = cv::Mat();
			printf("no face deteced!\n");
            return SDM_ERROR_FACENO;
        }
		printf("face rects %d \n" , mFaceRects.size() );
		if (mFaceRects.size() >= 1) {
			printf("mFaceRects 0   x:%d y:%d w:%d h %d \n", mFaceRects[0].x, mFaceRects[0].y, mFaceRects[0].height, mFaceRects[0].width);
		}
        faceBox = mFaceRects[0];
		printf("face area :%d\n", faceBox.area());
		printf("face area x:%d y:%d w:%d h %d \n", faceBox.x , faceBox.y, faceBox.height , faceBox.width );
        for(int i=1; i<mFaceRects.size(); i++){
            if(faceBox.area() < mFaceRects[i].area())
                faceBox = mFaceRects[i];
        }
	...
	printf("LinearRegressors.size() %d \n" , LinearRegressors.size() );
