 ...
 **/
#define GET_ARRAY_PARAM 0.23570226//(1.0f/sqrtf(18.0f))
   ...
   */
  {
    ...
    for (y = 0 ; y < (signed)self->hogHeight ; ++y) {
      for (x = 0 ; x < (signed)self->hogWidth ; ++x) {
		...
		float * oiter = features + y + self->hogHeight* x ;
        ...
        for (k = 0 ; k < self->numOrientations ; ++k) {
          ...
          *oiter = ha ;//0,1,2,3
          *(oiter + hogStride * self->numOrientations) = hb ;//4,5,6,7
          *(oiter + 2 * hogStride * self->numOrientations) = hc ;//8,9,10,11
          oiter += hogStride ;//1,2,3,4

        } /* next orientation */

        //case VlHogVariantUoctti :
        oiter += 2 * hogStride * self->numOrientations ;//+12 hogStride
        *oiter = GET_ARRAY_PARAM * t1 ; oiter += hogStride ;//13
        *oiter = GET_ARRAY_PARAM * t2 ; oiter += hogStride ;//14
        *oiter = GET_ARRAY_PARAM * t2 ; oiter += hogStride ;//15
        *oiter = GET_ARRAY_PARAM * t2 ; oiter += hogStride ;//16
